...
#define TIMEOUT_DURATION 100 // Define timeout duration for easy adjustment
...
uint8_t sensor_data[8] = { 0 };    // Array to hold sensor readings
uint8_t last_state_left = 0;       // Last state of the left sensor
uint8_t last_state_right = 0;      // Last state of the right sensor
uint8_t CROSS = 0;                 // Crossing counter

// LINE PROCESSING VARIABLES
uint8_t last_direction = 0;        // Last movement direction

// DATA PROCESSING VARIABLES
uint8_t history[8];                // Array to hold DT values for Phase 2
uint8_t DTIndex = 0;               // Index to keep track of the history array
uint8_t phase = 0;                 // Current phase: 0 - Init, 1 - Read Data
uint8_t count_up = 0;              // Counter for phase transitions
uint8_t enough_data = 0;           // Flag to indicate if enough data is collected

// CLOCK AND TIMING VARIABLES
uint8_t CK_set = 0;                // CK initialization flag
static uint8_t *CK = NULL;         // Pointer to CK (clock)
static uint8_t *DT = NULL;         // Pointer to DT (data)
...
void convert_array_to_string(uint8_t *array, uint8_t size, char *result, int result_size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        offset += snprintf(result + offset, result_size - offset, "%d", array[i]);
